...
from peft import (
    ...
    PromptTuningConfig,
    TaskType,
    PromptTuningLodimConfig,
    DPTPromptTuningConfig,
)
...
class ModelArguments:
    ...
    lodim_r: int = field(default=64, metadata={"help": "ULPT low dimension"})
    peft_type: str = field(
...
class DynamicTrainingArguments(Seq2SeqTrainingArguments):
    ...
    pretrained_path: Optional[str] = field(
        default=None,
        metadata={"help": "Path to pretrained embedding"}
    )
    save_base_embeddings: bool = field(
        default=True,
        metadata={"help": "Whether to save layernorm in lodimpt."}
    )
    num_random_proj: int = field(
        default=1,
        metadata={"help": "number of random projections"}
    )
...
def main():
    ...
    print('current seed:', training_args.seed)
    ...
    elif any(x in model_args.model_name_or_path for x in ["gpt", "llama", "opt", "bloomz"]):
    ...
    if model_args.peft_model_id:
        ...
        if task_type == TaskType.SEQ_CLS:
            ...
            model = AutoModelForSeq2SeqLM.from_pretrained(peft_config.base_model_name_or_path)
                                                  ...
                                                  or "bloomz" in model_args.model_name_or_path
                                                  or "opt" in model_args.model_name_or_path
                                                  ):
        ...
        if model_args.peft_type == "PROMPT_TUNING":
            ...
            peft_config = PromptTuningLoRAConfig(
                ...
                tokenizer_name_or_path=model_args.model_name_or_path,
                r=model_args.r,
                lodim_r=model_args.lodim_r,
                token_dim=model_args.hidden_size,
                max_length=data_args.max_seq_length,
                save_lora_embeddings=training_args.save_lora_embeddings,
            )
        elif model_args.peft_type == 'PROMPT_TUNING_LODIM':
            logger.info(f"\n\nLoading model for LODIM prompt tuning.\n\n")
            if training_args.pretrained_path != None:
                state_dict = torch.load(training_args.pretrained_path)

            peft_config = PromptTuningLodimConfig(
                task_type=task_type,
                prompt_tuning_init=PromptTuningInit.TEXT if model_args.text_init else PromptTuningInit.RANDOM,
                num_virtual_tokens=model_args.prefix_length,
                num_transformer_submodules=model_args.num_transformer_submodules,
                prompt_tuning_init_text=model_args.prompt_tuning_init_text,
                tokenizer_name_or_path=model_args.model_name_or_path,
                r=model_args.lodim_r,
                num_random_proj=training_args.num_random_proj,
                output_dir=training_args.output_dir,
            )
        
        elif model_args.peft_type == 'PROMPT_TUNING_DPT':
            logger.info(f"\n\nLoading model for DPT prompt tuning.\n\n")
            peft_config = DPTPromptTuningConfig(
                task_type=task_type,
                prompt_tuning_init=PromptTuningInit.TEXT if model_args.text_init else PromptTuningInit.RANDOM,
                num_virtual_tokens=model_args.prefix_length,
                num_transformer_submodules=model_args.num_transformer_submodules,
