...
class PromptTuningLoRAConfig(PromptLearningConfig):
    ...
    lodim_r: int = field(default=64, metadata={"help": "Lora low dimension"})
...
class PromptEmbeddingLoRA(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, config, word_embeddings):
        ...
        # for lodim
        self.embedding = torch.nn.Embedding(total_virtual_tokens, config.lodim_r)
        self.linear_layer = torch.nn.Linear(config.lodim_r, config.token_dim, bias=False)
        with torch.no_grad():
            torch.nn.init.xavier_uniform_(self.linear_layer.weight)
        for param in self.linear_layer.parameters():
            param.requires_grad = False
        self.base_embedding = BaseEmbedding(config.token_dim)
    ...
    def forward(self, indices):
        ...
        return self.base_embedding(self.linear_layer(prompt_embeddings))
