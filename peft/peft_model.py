...
PEFT_TYPE_TO_MODEL_MAPPING = {
    ...
    PeftType.PROMPT_TUNING_LODIM: PromptEmbeddingLodim,
    PeftType.PROMPT_TUNING_DPT: DPTPromptEmbedding,
}
...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _setup_prompt_encoder(self, adapter_name):
        ...
        elif config.peft_type == PeftType.PROMPT_TUNING_LODIM:
            prompt_encoder = PromptEmbeddingLodim(config, self.word_embeddings)
        elif config.peft_type == PeftType.PROMPT_TUNING_DPT:
            prompt_encoder = DPTPromptEmbedding(config, self.word_embeddings)
        ...
        try:
            if not isinstance(peft_config, PromptLearningConfig):
                ...
                if kwargs.get("attention_mask", None) is not None and peft_config.peft_type in [
                    ...
                    PeftType.PROMPT_TUNING_LODIM,
                    PeftType.PROMPT_TUNING_DPT,
                ]:
                    # concat prompt attention mask
...
class PeftModelForSeq2SeqLM(PeftModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        elif peft_config.peft_type == PeftType.PROMPT_TUNING or peft_config.peft_type == PeftType.PROMPT_TUNING_LODIM or peft_config.peft_type == PeftType.PROMPT_TUNING_DPT:
            if inputs_embeds is None:
                inputs_embeds = self.word_embeddings(input_ids)
